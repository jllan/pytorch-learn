...
#生成数据，unsqueeze函数是将一维数据变为二维，linspace(-1,1,100),在-1～1之间生成一百个数据，维度为1.
x = torch.unsqueeze(torch.linspace(-1, 1, 100), dim=1)
y = x.pow(1) + 0.2*torch.rand(x.size())  # m.pow(x)为m的x次方 后面加上的是噪声
...
optimizer = torch.optim.SGD(net.parameters(), lr=0.1)  # 优化神经网络的参数，并赋予学习率
criterion = torch.nn.MSELoss()  # MSEloss均方差
...
for t in range(200):
    ...
    if t % 5 == 0:
        ...
        plt.text(0.5, 0, 'epoch={}, loss={:.4f}'.format(t, loss.data[0]),
